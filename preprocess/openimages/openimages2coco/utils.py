...
def convert_instance_annotations(original_annotations, images, categories, start_index=0):
    ...
    for i in tqdm(range(num_instances), mininterval=0.5):
        ...
        if original_annotations_dict[csv_line]['IsGroupOf'] == '0':
            annotations.append(ann)
        else:
            n_isgroup += 1
    
    print('n_isgroup {}'.format(n_isgroup))
    return annotations
...
### dat code ###
def convert_instance_segmentation_annotations(original_segmentations, images, categories, original_mask_dir, segmentation_out_dir, start_index=0):
    
    original_annotations_dict = _list_to_dict(original_segmentations)
    ...
    annotated_attributes = [attr for attr in ['IsOccluded', 'IsTruncated', 'IsGroupOf', 'IsDepiction', 'IsInside'] if attr in original_segmentations[0]]
    ...
    for i in tqdm(range(num_instances), mininterval=0.5):
        ...
        xmin = float(original_annotations_dict[csv_line]['BoxXMin']) * imgs[image_id]['width']
        ymin = float(original_annotations_dict[csv_line]['BoxYMin']) * imgs[image_id]['height']
        xmax = float(original_annotations_dict[csv_line]['BoxXMax']) * imgs[image_id]['width']
        ymax = float(original_annotations_dict[csv_line]['BoxYMax']) * imgs[image_id]['height']
        dx = xmax - xmin
        dy = ymax - ymin
        ann['bbox'] = [round(a, 2) for a in [xmin , ymin, dx, dy]]
        ann['area'] = round(dx * dy, 2)
        
        mask_file = os.path.join(original_mask_dir,original_annotations_dict[csv_line]['MaskPath'])#_get_mask_file(original_annotations_dict[csv_line], original_mask_dir)
        # binary_mask = np.asarray(Image.open(mask_file).convert('1')).astype(np.uint8)
        # category_info = {'id': ann['category_id'], 'is_crowd': False}
        # img_size = (imgs[image_id]['width'], imgs[image_id]['height'])
        ...
        # append
        ann['iseg_file_name'] = mask_file#annotation_info['segmentation']
        ...
        ### is_group_of is in challenge-2019-validation-segmentation-bbox.csv but no challenge-2019-validation-segmentation-masks.csv
        # if original_annotations_dict[csv_line]['IsGroupOf'] == '0':
        #     annotations.append(ann)
        # else:
        #     n_isgroup += 1
...
#             binary_mask = np.asarray(Image.open(mask_file).convert('1')).astype(np.uint8)
#             category_info = {'id': segment_info['category_id'], 'is_crowd': False}
#             image_id = img['id']
...
#             # append
#             ann['segmentation'] = annotation_info['segmentation']
...
def filter_images_fast(images, annotations):
    image_ids = list(np.unique([ann['image_id'] for ann in annotations]))
    filtered_images = images#[img for img in images if img['id'] in image_ids]#
    return filtered_images
