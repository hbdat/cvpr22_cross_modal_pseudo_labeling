...
class TeacherUncertaintySTGeneralizedRCNN(nn.Module):
    """
    ...
    def __init__(self, cfg):
        super(TeacherUncertaintySTGeneralizedRCNN, self).__init__()
            ...
            if len(idxs_cap) > 0:
                ...
                try:
                    ...
                    for k in loss_pseudo:
                        if self.uncertainty:
                            if 'mask' not in k:
                                self.adaptive_lamb = self.roi_heads['mask'].avg_uncertain.detach()*self.lambda_pseudo_label #torch.clamp(0.01/self.roi_heads_student['mask'].avg_uncertain, max=self.lambda_pseudo_label)
