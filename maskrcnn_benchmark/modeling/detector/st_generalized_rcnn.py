...
class STGeneralizedRCNN(nn.Module):
    """
    ...
    def __init__(self, cfg):
        super(STGeneralizedRCNN, self).__init__()
        ...
        self.reweight = cfg.MODEL.REWEIGHT
    ...
    def forward(self, images, targets=None):
        ...
        if self.training:
            ...
            if len(idxs_cap) > 0:
                ...
                try:
                    _,_, loss_pseudo = self.roi_heads_student(cap_features, cap_proposals, pseudo_targets, compute_uncertain=self.uncertainty)

                    for k in loss_pseudo:
                        if self.uncertainty and self.reweight:
                            if 'mask' not in k:
                                self.adaptive_lamb = 0.01/self.roi_heads_student['mask'].avg_uncertain.detach() #torch.clamp(0.01/self.roi_heads_student['mask'].avg_uncertain, max=self.lambda_pseudo_label)
            ...
            if self.iter % 100 == 0:
                ...
                if self.uncertainty:
                    ...
                    for p in self.roi_heads_student['mask'].predictor.uncertain_pred.parameters():
                        norm_uncertainty += torch.norm(p)

                    print('norm_uncertainty',norm_uncertainty.item())
            self.iter += 1

            # stop uncertainty training
            if self.uncertainty and self.iter == self.uncertainty_train_iter:
                self.roi_heads_student['mask'].predictor.uncertain_pred.requires_grad_(requires_grad=False)
            ...
            embs = self.combine_embs(self.class_names, class_embs)#class_embs#
            self.roi_heads_student['box'].predictor.set_class_embeddings(embs)
            x, result_student, _ = self.roi_heads_student(features, proposals, targets, compute_uncertain=False, is_eval_func = True)
