...
class ROIBoxHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None):
        ...
        class_logits, box_regression, att_map = self.predictor(x)

        if not self.training:
            result , att_map = self.post_processor((class_logits, box_regression), proposals, att_map)
            return x, result, {}, att_map
...
def build_roi_box_head(cfg, in_channels):
    ...
    if cfg.MODEL.ROI_BOX_HEAD.WSDDN:
        raise Exception('Not Implemented cfg.MODEL.ROI_BOX_HEAD.WSDDN')
    else:
