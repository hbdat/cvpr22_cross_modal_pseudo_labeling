...
class PseudoMaskGeneralizedRCNN(nn.Module):
    """
    ...
    def __init__(self, cfg):
        super(PseudoMaskGeneralizedRCNN, self).__init__()
        ...
        self.resume = True#cfg.MODEL.RESUME
    ...
    def forward(self, images, targets=None):
        ...
        targets = [targets[0].to(images.tensors.device)]
        ...
        if len(idxs_cap) > 0:
            ...
            self.roi_heads_student.train()  #focus into training mode
            _, result_student, loss_pseudo = self.roi_heads_student(cap_features, cap_proposals, pseudo_targets, compute_uncertain=self.uncertainty)
            result_student[0].add_field('joined_class_names','/'.join(caps[0]))
        else:
            result_student = [BoxList(np.zeros((0,4)), targets[0].size, mode=targets[0].mode)]
