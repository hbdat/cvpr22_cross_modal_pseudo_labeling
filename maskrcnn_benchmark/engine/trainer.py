...
def do_train(
    ...
    for iteration, (images, targets, _) in enumerate(data_loader, start_iter):
        ...
        if hasattr(module,'exemplars'):
            if (iteration-1) % exemplar_sync_period == 0 or iteration == max_iter:
                module.save_exemplars()
            if (iteration-2) % exemplar_sync_period == 0 and iteration > 1:
